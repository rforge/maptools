unionSpatialPolygons <- function(SpP, IDs, threshold=NULL, avoidGEOS=FALSE, bound=1000L, avoidUnaryUnion=FALSE) {
    ...
    if (rgeosI && !avoidGEOS) {
        ...
        if (avoidUnaryUnion || version_GEOS0() < "3.3.0")
            res <- gUnionCascaded(spgeom=SpP, id=IDs, bound=bound)
        else
